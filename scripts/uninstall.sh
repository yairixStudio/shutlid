...
echo "1/8 restoring normal sleep"
sudo /usr/bin/pmset disablesleep 0

echo "2/8 removing the boot-time reset daemon"
sudo launchctl bootout system/com.yairix.shutlid.reset 2>/dev/null || true
sudo rm -f "$plist"

echo "3/8 removing the sudo rule"
sudo rm -f "$sudoers"
sudo visudo -c

echo "4/8 removing the command-line symlink"
